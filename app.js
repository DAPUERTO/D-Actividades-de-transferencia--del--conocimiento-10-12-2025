...
while (opcion !== '0') {
    ...
    opcion = prompt("Seleccione ejercicio: ");
    ...
    if (opcion === '1') {
        ...
        for (let i = 0; i < cantNotas; i++) {
            const nota = parseFloat(prompt(`Nota ${i + 1}: `));
            notas.push(nota);
        }
        ...
        for (let i = 0; i < cantA; i++) {
            ...
            catA.push({ id: i + 1, nombre, precio });
        }
        ...
        for (let i = 0; i < cantB; i++) {
            const nombre = prompt(`Nombre producto ${i + 1}: `);
            const precio = parseFloat(prompt(`Precio producto ${i + 1}: `));
            catB.push({ id: cantA + i + 1, nombre, precio });
        }
        ...
        const cliente = { nombre: nombreCliente, correo };
